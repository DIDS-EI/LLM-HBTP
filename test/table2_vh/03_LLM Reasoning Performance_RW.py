...
def validate_goal(env, chosen_goal, n,grounding_objs=set()):
    ...
    while not success and reflect_time <= 3:
        ...
        if reflect_time == 0:
            ...
            ACT_PREDICATES = {"MoveTo", "PutDown", "PickUp", "Make", "Clean",
            "Turn_ < appliance_furnishing > _On", "Turn_ < appliance_furnishing > _Off", "Turn_ACTemperature_Up", "Turn_ACTemperature_Down"}
            ...
            reflect_prompt = (
                ...
                "Note that you have not used the following Action Predicates: \"{not_use_pred_str}\" "
            ...
            reflect_prompt = reflect_prompt.format(goals=d['Goals'], have_finished=have_finished_str,
                                                   ...
                                                   not_use_pred_str=not_use_pred_str)
    ...
    if not success:
        ...
        all_obj_pred = list(set(llm_key_obj) | set(grounding_objs))
