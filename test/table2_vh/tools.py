...
#     cur_cond_set |= {f'IsClose({arg})' for arg in VHAction.CAN_OPEN}
#     cur_cond_set |= {f'IsSwitchedOff({arg})' for arg in VHAction.HAS_SWITCH}
#     cur_cond_set |= {f'IsUnplugged({arg})' for arg in VHAction.HAS_PLUG}
...
def execute_algorithm(algo, goal_set, cur_cond_set):
    ...
    error, state, act_num, current_cost, record_act_ls = algo.execute_bt(\
        goal_set[0], cur_cond_set, verbose=True)
