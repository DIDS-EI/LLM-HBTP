...
class BTExpInterface:
    def __init__(self, behavior_lib, cur_cond_set, priority_act_ls=[], key_predicates=[], key_objects=[], selected_algorithm="opt",
                 ...
                 heuristic_choice=-1,get_bt_summary=False):
        """
        ...
        if mode == "big":
            ...
            self.actions = action_list
            print(f"Customized small action space: {len(self.actions)} actions have been collected.")
            ...
            print(
                f"Select the small action space, considering only objects: {len(self.actions)} actions have been collected.")
            ...
            # print(
            #     f"Select the small action space, considering predicates and objects: {len(self.actions)} actions have been collected.")
            print(
                f"Small action space: {len(self.actions)} actions have been collected.")
        ...
        self.get_bt_summary = get_bt_summary
    ...
    def process(self, goal):
        ...
        if self.selected_algorithm == "opt":
            ...
            self.algo = OptBTExpAlgorithm(verbose=False, \
                                          ...
                                          heuristic_choice = self.heuristic_choice,get_bt_summary=self.get_bt_summary)
...
def collect_action_nodes(behavior_lib):
    ...
    print(f"A total of {len(action_list)} instantiated actions have been collected.")
    print(f"A total of {can_expand_ored} action predicates have been collected.")
