...
class OptBTExpAlgorithm:
    def __init__(self, verbose=False, llm_reflect=False, llm=None, messages=None, priority_act_ls=None, time_limit=None, \
                 consider_priopity=False, heuristic_choice=-1,get_bt_summary=False):
        self.bt = None
        self.start = None
        self.goal = None
        self.actions = None
    ...
    def get_bt_summary_fun(self,goal_cond_act_pair):
        # 获取最远的 top 5 序列的动作序列
        ...
        act_tree_string = self.transfer_pair_node_to_act_tree(path_nodes=path_nodes, root_pair=goal_cond_act_pair)
        # print(act_tree_string)
        return act_tree_string



    def run_algorithm_selTree(self, start, goal, actions, merge_time=9999999):
        '''
        ...
        for key, value in goal_cond_act_pair.pact_dic.items():
            if key in self.act_cost_dic:  # Add check to avoid KeyError
        ...
        while len(self.nodes) != 0:
            ...
            # 获取最远的 top 5 序列的动作序列
            if self.get_bt_summary == True:
                self.bt_summary = self.get_bt_summary_fun(goal_cond_act_pair=goal_cond_act_pair)
                # print(self.bt_summary)
